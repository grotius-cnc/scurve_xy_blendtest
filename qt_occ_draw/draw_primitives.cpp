...
bool draw_primitives::is_point_on_arc(gp_Pnt p0, gp_Pnt pw, gp_Pnt p1, gp_Pnt p){
    Eigen::Vector3d p1_(p0.X(),p0.Y(),p0.Z());
    Eigen::Vector3d p2_(pw.X(),pw.Y(),pw.Z());
    Eigen::Vector3d p3_(p1.X(),p1.Y(),p1.Z());
    Eigen::Vector3d p4_(p.X(),p.Y(),p.Z());
    ...
    //! Center of arc.
    Eigen::Vector3d center = p1_ + v1*k1 + v2*k2;

    double radius=p0.Distance({center.x(),center.y(),center.z()});
    if(p.Distance({center.x(),center.y(),center.z()})!=radius){
       // std::cout<<"radius differs."<<std::endl;
       // return 0;
    }

    //! Arc angle.
    Eigen::Vector3d va=(p1_-center).normalized();
    Eigen::Vector3d vb=(p3_-center).normalized();
    Eigen::Vector3d vc=(p4_-center).normalized();
    ...
    // Calculate angle p0 to p.
    //! This can be a negative angle if angle > 180 degrrees. Solution is below.
    angle=acos(va.dot(vc));
    Eigen::Vector3d vac=va.cross(vc);
    dot=vac.dot(arc_normal);
    arcAngleNegative=false;
    if(dot<0){
        double diff=M_PI-angle;
        angle=M_PI+diff;
        arcAngleNegative=true;
    }
    double angle_deg_first=angle*RadtoDeg;


    // Calculate p to p1.
    angle=acos(vc.dot(vb));
    Eigen::Vector3d vcb=vc.cross(vb);
    dot=vcb.dot(arc_normal);
    arcAngleNegative=false;
    if(dot<0){
        double diff=M_PI-angle;
        angle=M_PI+diff;
    ...
    double angle_deg_second=angle*RadtoDeg;


    if(angle_deg_total==angle_deg_first+angle_deg_second){
      std::cout<<"point is on arc"<<std::endl;
      return 1;
    } else {
        std::cout<<"point is not on arc"<<std::endl;
    }
    // std::cout<<"angle total:"<<angle_deg_first+angle_deg_second<<std::endl;
    return 0;
}
    ...
    if(r-offset>0){
        if(intersect_3d_line_sphere(pc,po1,po2,r-offset,pc_offset_sphere)){ //! Centerpoint offset sphere.
            if(intersect_3d_sphere_sphere(pc_offset_sphere,offset,pc,r,pi_arc_contour)){ //! Intersect contour arc with offset spehe.
               if(is_point_on_arc(p3,pw,p4,pi_arc_contour)){
